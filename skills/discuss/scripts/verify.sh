...
echo
echo "── 覆盖范围声明 ──"
# R-041。本自检的参与者名单派生自 invoke.py --list-agents,而那是**适配库登记
# 名单**(只含 CLI)。API 参与者写在用户名册里、由壳每轮转写成 sidecar,自检
# 运行时它压根不存在,所以这里检不到它们的凭证。
#
# 补不了能力就不要给出超出能力的绿灯 —— 这个仓库对「绿灯自检 + 凭空缺席」
# 有明确的痛(见上面那段关于 gemini 信任目录的注释),那是公认最难查的失败模式。
note "本自检**不覆盖** API 参与者的凭证检查"
echo "      名册里的 API 参与者(api_key_env 指向的环境变量)在讨论调用那一刻才检查;"
echo "      要提前确认,直接 echo \$YOUR_API_KEY_ENV 看是否非空。"
