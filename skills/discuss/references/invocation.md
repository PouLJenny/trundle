...
> 那个能"解决问题"的环境变量名,本仓库的**代码**里一处都没有。
> 唯一写出它的是 `SKILL.md` 的铁律,那一处是为了指名禁止 —— 主持人 Claude
> 才是可能被要求去绕过的那一方,它需要认得出这是什么。
> 看到它请不要加进代码——它让症状消失的方式是把延迟涨 10 倍。
...
有事件流的 agent 跑飞了会先被空闲超时砍,墙钟只是兜底;**无事件流的 agent 中途没有任何信号能区分「在想」和「死了」**,墙钟是它唯一的护栏。

| agent | max_wall | 理由 |
|---|---|---|
| codex / gemini / claude | 540s | 有空闲超时兜底 |
| dsh | 540s | 无流,墙钟是唯一护栏。曾收紧到 300s,实际使用中重 prompt 会超过它,放宽到全局同款 540 |

540 是上限而不是随意值:Bash 工具自己在 600s 开枪,invoke.py 必须留余量先收尾,否则整轮分段输出和失败分类全丢。所以任何 agent 的 max_wall 都不能设到 600。
...
```
··· 5s  │ dsh ▸ 运行中(无进度事件) · 已跑 5s(上限 540s)  │ codex ▸ 思考中
··· 16s │ dsh ▸ 运行中(无进度事件) · 已跑 16s(上限 540s) │ codex ▸ 发言中 413字
